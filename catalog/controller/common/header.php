...
class ControllerCommonHeader extends Controller {
	protected function index() {
        ...
        // instrument logo and name

        global $loader, $registry;
        $loader->model('setting/store');
        $model = $registry->get('model_setting_store');
        $inst_names = $model->getStore(1);
        $this->data['home_inst'] = $inst_names['url'];
        $this->data['name_inst'] = $inst_names['name'];

        $loader->model('setting/setting');
        $model = $registry->get('model_setting_setting');
        $resultImage=$model->getSetting('config',1, 'config_logo');

        if ($resultImage['config_logo']) {
            $this->data['logo_inst'] = $server . 'image/' . $resultImage['config_logo'];
        } else {
            $this->data['logo_inst'] = '';
