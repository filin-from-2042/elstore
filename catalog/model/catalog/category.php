...
// TODO: добавить очистку кэша после выгрузки
        ...
        $categories = $this->cache->get('categories.'.$parent_id.'.' . $cStoreId . '.' . $cLanguageId);
        if(!$categories){
		    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . $cLanguageId . "' AND c2s.store_id = '" . $cStoreId . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)");
            $categories = $query->rows;
            $this->cache->set('categories.'.$parent_id.'.' . $cStoreId . '.' . $cLanguageId, $categories);
        }
		return $categories;
	}
