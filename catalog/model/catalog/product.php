...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
        ...
        $cStoreId = (int)$this->config->get('config_store_id');
        $cLanguageId = (int)$this->config->get('config_language_id');
        $productData = $this->cache->get('getProduct.'.$product_id.'.' . $cStoreId . '.' . $cLanguageId.'.'.$customer_group_id);
        if(!$productData){
            $query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.measure AS measure, ".
                ...
                " WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
            ...
            if ($query->num_rows) {
                $productData =  array(
                 ...
                 $this->cache->set('getProduct.'.$product_id.'.' . $cStoreId . '.' . $cLanguageId.'.'.$customer_group_id,$productData);
            } else {
                $productData = false;
            }
        }
