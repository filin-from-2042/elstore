...
class ModelCatalogFilter extends Model {
  ...
  public function addOption($data) {
    ...
    if (isset($data['option_value']['insert'])) {
			foreach ($data['option_value']['insert'] as $value) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "category_option_value SET option_id = '" . (int)$option_id . "', sort_order = '" . (int)$value['sort_order'] . "'");
  ...
  public function editOption($option_id, $data) {
    ...
    if (isset($data['option_value'])) {
			if (isset($data['option_value']['update'])) {
				foreach ($data['option_value']['update'] as $value_id => $value) {
	        $this->db->query("INSERT INTO " . DB_PREFIX . "category_option_value SET option_id = '" . (int)$option_id . "', sort_order = '" . (int)$value['sort_order'] . "', value_id = '" . (int)$value_id . "'");
  ...
  public function copyAttributesToFilter($data = array()) {
    ...
    $this->db->query("INSERT INTO " . DB_PREFIX . "category_option (option_id, status, type, sort_order) SELECT attribute_id, '1' AS status, '" . $this->db->escape($data['type']) . "' AS type, sort_order FROM " . DB_PREFIX . "attribute");
    $this->db->query("INSERT INTO " . DB_PREFIX . "category_option_description (option_id, language_id, name) SELECT attribute_id, language_id, name FROM " . DB_PREFIX . "attribute_description");

		$this->db->query("SET @row=0");
		$this->db->query("INSERT INTO " . DB_PREFIX . "category_option_value (value_id, option_id) SELECT @row:=@row+1, attribute_id FROM " . DB_PREFIX . "product_attribute GROUP BY attribute_id, `text` ORDER BY attribute_id, `text`");

		$this->db->query("SET @row=0");
		$this->db->query("INSERT INTO " . DB_PREFIX . "category_option_value_description (value_id, language_id, option_id, name) SELECT @row:=@row+1, language_id, attribute_id, text FROM " . DB_PREFIX . "product_attribute GROUP BY attribute_id, `text` ORDER BY attribute_id, `text`");
    ...
    $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_value (product_id, value_id, option_id) SELECT pa.product_id, covd.value_id, pa.attribute_id FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "category_option_value_description covd ON (covd.name = pa.text) WHERE pa.language_id = '" . (int)$this->config->get('config_language_id') . "' AND covd.language_id = '" . (int)$this->config->get('config_language_id') . "'");

		$this->db->query("INSERT INTO " . DB_PREFIX . "category_option_to_category (option_id, category_id) SELECT p2v.option_id, p2c.category_id FROM " . DB_PREFIX . "product_to_value p2v LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2c.product_id = p2v.product_id) WHERE p2c.category_id != '0' GROUP BY p2v.option_id, p2c.category_id");
